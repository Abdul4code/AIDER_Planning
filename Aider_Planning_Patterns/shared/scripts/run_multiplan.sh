...
# Runs multi-plan selection using an external sidecar harness.
# Follows Self-Consistency approach: generate multiple diverse plans via temperature sampling,
# evaluate via tests, and select optimal plan using majority vote.
...
HARNESS_SCRIPT="MultiPlan/scripts/multiplan_harness.py"
...
if [[ ! -f "$HARNESS_SCRIPT" ]]; then
	echo "ERROR: multiplan harness missing: $HARNESS_SCRIPT" >&2
	exit 4
fi
...
run_variant_name="multiplan"
results_dir="MultiPlan/results"
...
multiplan_tries="${AIDER_BENCH_MULTIPLAN_TRIES:-2}"
multiplan_num_plans="${AIDER_BENCH_MULTIPLAN_NUM_PLANS:-4}"
multiplan_task_timeout_seconds="${AIDER_BENCH_MULTIPLAN_TASK_TIMEOUT_SECONDS:-900}"
multiplan_llm_timeout="${AIDER_BENCH_MULTIPLAN_LLM_TIMEOUT:-60}"
multiplan_retry_timeout="${AIDER_BENCH_MULTIPLAN_RETRY_TIMEOUT:-30}"

harness_cmd=(
	/workspace/MultiPlan/scripts/multiplan_harness.py
	--run-name "$run_name"
	--model "$model_arg"
	--edit-format "$AIDER_BENCH_EDIT_FORMAT"
	--threads "$AIDER_BENCH_THREADS"
	--tries "$multiplan_tries"
	--num-plans "$multiplan_num_plans"
...
echo "== Running multi-plan selection benchmark (sidecar harness) =="
echo "Run: $run_name"
...
echo "Plans per try: $multiplan_num_plans"
...
docker run --rm \
	...
	-e AIDER_BENCH_EXTRA_INSTRUCTIONS="$MULTIPLAN_PROMPT" \
	-e AIDER_BENCH_LLM_TIMEOUT="$multiplan_llm_timeout" \
	-e AIDER_BENCH_RETRY_TIMEOUT="$multiplan_retry_timeout" \
	-e AIDER_BENCH_TASK_TIMEOUT_SECONDS="$multiplan_task_timeout_seconds" \
...
if [[ "$docker_rc" -ne 0 ]]; then
	echo "ERROR: MultiPlan run failed. See logs:" >&2
...
echo "OK: MultiPlan run completed. Logs at $run_dir"
...
# Optional: check against baseline
require_baseline_parity="${AIDER_BENCH_REQUIRE_BASELINE_PARITY:-0}"
if [[ "$require_baseline_parity" == "1" ]]; then
	baseline_summary_json="${AIDER_BENCH_BASELINE_SUMMARY_JSON:-}"
	if [[ -z "$baseline_summary_json" ]]; then
		echo "WARN: baseline parity check: AIDER_BENCH_BASELINE_SUMMARY_JSON not set, skipping" >&2
	else
		echo "== Checking multiplan accuracy vs baseline =="
		"$PY" - <<PY
import json
import sys

baseline_path = "$baseline_summary_json"
multiplan_path = "$summary_json"
...
try:
	with open(multiplan_path) as f:
		multiplan = json.load(f)
	multiplan_pass_rate = multiplan.get("pass_rate", 0.0)
except Exception as e:
	print(f"ERROR: Could not read multiplan results: {e}", file=sys.stderr)
	sys.exit(1)

print(f"Baseline pass rate:  {baseline_pass_rate:.1%}")
print(f"MultiPlan pass rate: {multiplan_pass_rate:.1%}")

if multiplan_pass_rate >= baseline_pass_rate:
	print(f"OK: MultiPlan met or exceeded baseline ({multiplan_pass_rate:.1%} >= {baseline_pass_rate:.1%})")
else:
	diff = baseline_pass_rate - multiplan_pass_rate
	print(f"WARN: MultiPlan regressed by {diff:.1%} ({multiplan_pass_rate:.1%} < {baseline_pass_rate:.1%})")
	sys.exit(7)
PY
	fi
fi
